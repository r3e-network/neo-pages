...
interface AcceptOrganizationInviteCardProps {
  token: string;
  email: string;
  status: string;
}

export function AcceptOrganizationInviteCard({ token, email, status }: AcceptOrganizationInviteCardProps) {
  ...
  async function acceptInvite() {
    ...
    try {
      const response = await fetch(`/api/organization-invites/${token}/accept`, { method: 'POST' });
